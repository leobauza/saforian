...
$ch = curl_init();  
curl_setopt($ch, CURLOPT_URL, 'http://api.twitter.com/1/statuses/user_timeline/saforian.json?count=1');
...
$tweet = $decode[$i]["text"];
	...
	$status = preg_replace('/(@\w+)/', '<a href="http://twitter.com/$1" target="_blank">$1</a>', $status);
	if(isset($html)){
		$html .= "<li class=\"tweet\"><div class=\"inner-tweet\"><span>".$status."</span>";
	} else {
		$html = "<li class=\"tweet\"><div class=\"inner-tweet\"><span>".$status."</span><br>";
	}
