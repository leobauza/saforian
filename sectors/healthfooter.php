...
<section id="nonprofit-form">
	<div class="std-block">
		<h1>Want a demo of our health tools?</h1>
		<div class='row-fluid'>
			<article class='span6'>
			...
			<article class="span6 l padbottom">
				<p class="padformtext">We’ve been busy working on tools to advance your healthcare company. Contact us to learn how we can make your digital processes more efficient and your infrastructure less costly.</p>
			</article>
		</div>
	</div>
	...
	<script>
		...
		$("#submit").click(function(e) {
			...
			if($(this).closest('form').find(".LV_valid").length && !$(this).closest('form').find(".LV_invalid").length) {
				e.preventDefault();
				var url = "../contact-health.php"; // the script where you handle the form input.
				$.ajax({
					type: "POST",
					url: url,
